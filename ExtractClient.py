...
def parseTimeAndId(line):
    match = re.search(': +(\d+\.\d) ns .* WriteRpc (0x[0-9a-f]+)', line)
...
def processFile(f):
    ...
    for line in f:
        line = line.strip()
        if "WriteRpc" not in line: continue
        # Write is being sent out
        if "being constructed" in line:
            timestamp, ID = parseTimeAndId(line)
            writeIDs[ID] = timestamp
            continue
        if "just got response from server" in line:
            timestamp, ID = parseTimeAndId(line)
            if ID in writeIDs:
    ...
    if len(sys.argv) == 1:
        ...
        for f in sys.argv[1:]:
            try:
                ...
                traceback.print_exc()
                return
    print("Average: %f" % (sum(numbers) / len(numbers)))
